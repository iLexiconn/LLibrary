    ...
    public void markDirty(TileEntity tile) {
        ...
        if (tile instanceof ITrackableTile) {
            if (tile.getWorld().isRemote) {
                LLibrary.NETWORK_WRAPPER.sendToServer(new TileTrackMessage(tile.getPos(), (ITrackableTile) tile));
            } else {
                LLibrary.NETWORK_WRAPPER.sendToAll(new TileTrackMessage(tile.getPos(), (ITrackableTile) tile));
            }
        }
    }
    ...
    public void updateTrackers(World world) {
        ...
        for (Map.Entry<BlockPos, ITrackableTile> entry : new HashMap<>(trackedTiles).entrySet()) {
            ...
            if (trackingFrequency == 0 || ticks % trackingFrequency == 0) {
                if (this.firstTick.contains(trackedTile) || world.getTileEntity(pos) == trackedTile) {
